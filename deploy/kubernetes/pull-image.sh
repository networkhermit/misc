...
get_image_list () {
    ...
    mapfile -O ${#arr_ref[@]} -t arr_ref < <(curl --fail --location --silent --show-error https://raw.githubusercontent.com/kubernetes/kube-state-metrics/v2.2.4/examples/standard/deployment.yaml | awk '/\<image:\s*k8s.gcr.io\// { print $NF }')
